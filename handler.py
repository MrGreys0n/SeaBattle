...
PHRASES_FOR_USERS_TURN = ['Сейчас ваш ход, не надо поддаваться.',
                          ...
                          'Ходите, а я пока продумаю свои следующие ходы.']
...
def handle_dialog(request, response, user_storage):
    ...
    if request.is_new_session:
        ...
        # Приветствие
        response.set_text('Привет! Играем в морскую битву. Каждая клетка обозначается алфавитной буквой по горизонтали '
                          '(от "А" до "К", исключая "Ё" и "Й", слева направо) и цифрой по вертикали '
                          '(от 1 до 10 сверху вниз). Мои корабли уже расставлены.\nНабор кораблей таков:\n1 четырёхпалубный;\n2 трёхпалубных;\n3 двухпалубных;\n4 однопалубных.\nПо вашей готовности атакуйте. Чтобы '
                          'провести атаку скажите или введите координаты.\n'
                          'Для начала новой игры наберите "Новая игра" или "Выход".\n'
                          'Для вызова справки наберите "Помощь".\n'
                          'Ах да, вам понадобится листик и карандаш или ручка чтобы нарисовать поле.\n'
                          'Чтобы увидеть поле, наберите "Показать поле". К сожалению, на данный момент отображение работает не совсем '
                          'корректно - потопленный корабль и клетка, в которую был совершён выстрел, обозначаются одинаково - "O". '
                          'Раненый корабль обозначается "Х"')
...
# одинаково показаны потопленные корабли и прострелянные клетки, в будущем надо исправить
            ...
            # Если ходит Алиса
            elif user_message.lower() in HELP_WORDS:
                response.set_text('Чтобы выстрелить, надо назвать координаты клетки.\n'
                                  "Координаты клетки обозначаются буквой (от А до К, исключая Ё и Й) "
                                  "и числом (от 1 до 10) для поля 10 на 10 клеток. Пример - А1.\n"
                                  'Если я промазала, скажите "мимо", если попала - "попала", а если убила - "убила".\n'
                                  'Если хотите увидеть поле, наберите "Показать поле"')
            elif user_message.lower() in WHAT_CAN_U_DO_WORDS:
                response.set_text('Привет! Играем в морской бой. Каждая клетка обозначается алфавитной буквой по горизонтали '
                                  ...
                                  'Для начала новой игры наберите "Новая игра" или "Выход".\n'
                                  'Для вызова справки наберите "Помощь"\n'
                                  'Для отображения поля наберите "Показать поле"')
                ...
                if user_message in KILLED_WORDS:
                    ...
                    response.set_text('Чтобы выстрелить, надо назвать координаты клетки'
                                      "Координаты клетки обозначаются буквой (от А до К, исключая Ё и Й) "
                                      "и числом (от 1 до 10) для поля 10 на 10 клеток. Пример - А1. "
                                      'Если я промазала, скажи "мимо", если попала - "попала", а если убила - "убила".')
...
def alice_fires(user_data, happened):
    ...
    if happened == "убил" or happened == "ранил":
        ...
        if not (len(user_data["Target"]) + 1 in user_data["users_ships"]):
            delete_ship()
            return "Максимальный размер корабля на данный момент {} клетки. ".format(max(user_data["users_ships"]) + 1)
